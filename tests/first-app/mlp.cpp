...
* NArray NArray::RepMat(NArray m, vector<size_t> reptimes);
*     // This function repeat the original matrix m by the times given on each dimension.
*     // For example:
*     // If m is a 3x5 matrix, m1 = NArray::RepMat(m, {1, 2}); then m1 is a 3x10 matrix.
*
  ...
  for(int i = 0; i < num_epochs; i++) {
    for(size_t j = 0; j < num_minibatches; j++) {
      ...
      for(size_t k = 1; k < num_layers; ++k) {
        acts[k] = weights[k] * acts[k-1] + layers[k].bias.RepMat({1, num_minibatches});
      ...
      for(size_t k = 1; k < num_layers; ++k) { // no input layer
        layers[k].bias -= epsB * Reduction::Sum(sens[k], 1) / minibatch_size;
      }
      // Update weight
