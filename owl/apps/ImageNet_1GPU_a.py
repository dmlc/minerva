...
def train_network(bias, weights, data, label,
    ...
    for i in xrange(num_epochs):
        ...
        for j in xrange(num_minibatches):
            ...
            weights[7] -= eps_w * sens[16] * acts[15].trans()
            bias[7] -= eps_b * sens[16].sum(1)

            weights[6] -= eps_w * sens[15] * acts[14].trans()
            bias[6] -= eps_b * sens[15].sum(1)

            weights[5] -= eps_w * sens[14] * re_acts13.trans()
            bias[5] -= eps_b * sens[14].sum(1)

            weights[4] -= eps_w * conv_backward_filter(sens[11], acts[10], conv_info(1, 1, 1, 1))
            bias[4] -= eps_b * conv_backward_bias(sens[11])

            weights[3] -= eps_w * conv_backward_filter(sens[9], acts[8], conv_info(1, 1, 1, 1))
            bias[3] -= eps_b * conv_backward_bias(sens[9])

            weights[2] -= eps_w * conv_backward_filter(sens[7], acts[6], conv_info(1, 1, 1, 1))
            bias[2] -= eps_b * conv_backward_bias(sens[7])
