...
if __name__ == "__main__":
    ...
    builder.init_net_from_file(owl_net, sys.argv[3], int(sys.argv[4]))
    ...
    for iteridx in range(owl_net.solver.max_iter):
        ...
        '''
        if (iteridx + 1) % 10 == 0:
            builder.save_net_to_file(owl_net, sys.argv[3], 1)
        '''
        if (iteridx + 1) % owl_net.solver.snapshot == 0:
            builder.save_net_to_file(owl_net, sys.argv[3], (iteridx + 1) / owl_net.solver.snapshot + int(sys.argv[4]))
